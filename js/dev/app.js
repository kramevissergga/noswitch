...
function menuInit() {
  document.addEventListener("click", function(e) {
    if (bodyLockStatus) {
      ...
      if (e.target.closest("[data-chat-history-toggle]")) {
        document.documentElement.toggleAttribute("data-chat-history-open");
        setTimeout(() => {
          document.documentElement.hasAttribute("data-chat-history-open") ? bodyLock(0) : bodyUnlock(0);
        }, 0);
      } else if (!e.target.closest(".chat-history") && document.documentElement.hasAttribute("data-chat-history-open")) {
        bodyUnlock(0);
        document.documentElement.removeAttribute("data-chat-history-open");
      }
      if (e.target.closest("[data-filters-toggle]")) {
        document.documentElement.toggleAttribute("data-filters-open");
        setTimeout(() => {
          document.documentElement.hasAttribute("data-filters-open") ? bodyLock(0) : bodyUnlock(0);
        }, 0);
      } else if (!e.target.closest(".filters-catalog") && document.documentElement.hasAttribute("data-filters-open")) {
        bodyUnlock(0);
        document.documentElement.removeAttribute("data-filters-open");
      }
    }
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".field__pass-btn")) {
    const button = e.target;
    const fieldContainer = button.closest(".field__pass");
    const input = fieldContainer.querySelector("input");
    if (input.type === "password") {
      input.type = "text";
      button.classList.remove("__sprite-eye");
      button.classList.add("_sprite-eye-hide");
    } else {
      input.type = "password";
      button.classList.remove("_sprite-eye-hide");
      button.classList.add("_sprite-eye");
    }
  }
});
const autosize = () => {
...
const resultItems = document.querySelectorAll(".item-result");
resultItems.forEach((item) => {
  const scrollButton = item.querySelector(".item-result__down");
  const scrollContainer = item.querySelector(".item-result__scroll");
  if (scrollButton && scrollContainer) {
    const sb = SimpleBar.instances.get(scrollContainer);
    if (sb) {
      const scrollElement = sb.getScrollElement();
      scrollButton.addEventListener("click", () => {
        const scrollStep = scrollElement.clientHeight * 0.8;
        const newPosition = scrollElement.scrollTop + scrollStep;
      ...
      scrollElement.addEventListener("scroll", () => {
        const isEnd = scrollElement.scrollHeight - scrollElement.scrollTop <= scrollElement.clientHeight + 1;
        isEnd ? item.classList.add("--scrolled") : item.classList.remove("--scrolled");
      });
    }
  }
});
